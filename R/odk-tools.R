...
odk_to_stata <- function(survey, choices, language = NULL, df, file = "codebook.html") {
  ...
  choices.st$list_name <- as.character(choices.st$list_name)
  choices.st$mash <- as.character(choices.st$mash)
  # Smash up even further
  choices.st <- aggregate(. ~ list_name, choices.st, function(x) paste0(x[1]," ", x[2]))
  # Change name of choices.st to merge nicely with the survey
  names(choices.st) <- c("type", "smash")
  # Need to remove tpyes: start, today, deviceid,  date, text, integer, decimal
  survey.st <- subset(survey.st, !(survey.st$type %in% c("start", "today", "deviceid",
  ...
  # Apply value labels
  app_val_labs <- paste0("label values ", survey.st$name, " ", survey.st$type)

  # Create variable labels
  var_labs <- paste0("label variable ", survey.st$name, ' "', survey.st$label, '"')

  # escape underscores
  # val_labs <- str_replace_all(val_labs, fixed("_"), "\\_")
  # app_val_labs <- str_replace_all(app_val_labs, fixed("_"), "\\_")
  # var_labs <- str_replace_all(var_labs, fixed("_"), "\\_")

  st <- rbind(val_labs, app_val_labs, var_labs)
